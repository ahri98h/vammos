...
router.post('/churn-analysis', async (req, res) => {
  try {
    const report = await ReportsService.generateChurnReport();
    res.status(201).json(report);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
router.post('/schedule', async (req, res) => {
  try {
    ...
    const schedule = await ReportsService.scheduleReportRecurrence({
      type,
      frequency,
      recipients,
      format
...
router.post('/comparison', async (req, res) => {
  try {
    ...
    const comparison = await ReportsService.compareMetricsPeriods(
      metric,
      period1Start,
      period1End,
      period2Start,
